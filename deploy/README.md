...
On macOS, `service_scope` defaults to `user`. Set it to `system` only for a reviewed service that must run before interactive login. The deployer then installs a root-owned plist in `/Library/LaunchDaemons`, with `UserName` set to the deploying account, and manages it in launchd's `system` domain. Installation and lifecycle changes use non-interactive `sudo`; they fail clearly until administrator authorization is arranged. A project may set `launchd_admin_helper` to a root-owned, allowlisting helper rather than granting the deploy account broad passwordless access to `launchctl`, `install`, or `rm`. When moving from user to system scope, the deployer unloads the same-label user agent only after privileged access has passed its preflight, restores it if installation fails, and removes its plist after the LaunchDaemon is healthy. Linux continues to use user systemd and rejects no existing configurations.
...
`./deploy/node_project.zsh journey-planner` selects the `train-track-journey-planner` project and deploys to `mini` by default. `train-track-planner` is an alias for the same project. The checkout is `/Users/mwagstaff/dev/train-track-planner`, the system LaunchDaemon is `com.train-track-planner.api`, and the monitoring job is `train-track-planner`. Use `--no-tail` for a one-shot deploy. There is only one planner service definition. `metrics_port: false` keeps full deploys from attempting a Grafana import; Sky scrapes the authenticated Funnel endpoint through the dedicated monitoring setup. Run `deploy/install_mini_boot_services.zsh` once to install the restricted administrator helper, replace the old user agent, repair and enable the system Funnel job, and deploy the planner in system scope. The helper accepts only the reviewed planner and Funnel plists; it avoids broad passwordless `launchctl`, file installation, or deletion rights. FileVault must still unlock the disk before services using `/Users/mwagstaff` can start.
